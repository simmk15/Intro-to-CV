...
def apply(img,t):
    ...
    if t=="CARTOON":
        ...
        e=cv2.adaptiveThreshold(cv2.mediaBlur(g,7),255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,9,2)
        c=cv2.bilateralFilter(img,9,75,75)
        return cv2.bitwise_and(c,c,mask=e)
    return img
cap=cv2.VideoCapture(0)
if not cap.isOpened():print("Error! Could not access the web cam. "); exit()
